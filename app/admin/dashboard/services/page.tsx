  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--admin-text)' }}>Dịch vụ</h1>
          <p style={{ color: 'var(--admin-text-light)', fontSize: '0.8125rem' }}>{services.length} dịch vụ</p>
        </div>
          ...
          <div key={s.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
              <span style={{
                ...
                background: `${s.color === 'green' ? '#22c55e' : s.color === 'blue' ? '#3b82f6' : s.color === 'amber' ? '#f59e0b' : s.color === 'purple' ? '#8b5cf6' : 'var(--admin-text-light)'}20`,
                color: s.color === 'green' ? '#4ade80' : s.color === 'blue' ? '#60a5fa' : s.color === 'amber' ? '#fbbf24' : s.color === 'purple' ? '#a78bfa' : 'var(--admin-text-muted)',
              }}>{s.icon}</span>
              <div style={{ display: 'flex', gap: '0.375rem' }}>
                <button onClick={() => openEdit(s)} style={{
                  background: 'var(--admin-border)', border: 'none', borderRadius: '6px',
                  padding: '4px 6px', cursor: 'pointer', color: 'var(--admin-text-muted)',
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--admin-modal-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--admin-surface)', border: '1px solid #334155', borderRadius: '12px', padding: '1.5rem', maxWidth: '400px', width: '90%' }}>
            <h3 style={{ color: 'var(--admin-text)', marginBottom: '0.5rem' }}>Xóa dịch vụ?</h3>
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--admin-modal-bg)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 100, overflowY: 'auto', padding: '2rem 1rem' }}>
          <div style={{ background: 'var(--admin-surface)', border: '1px solid #334155', borderRadius: '12px', padding: '1.5rem', maxWidth: '600px', width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ color: 'var(--admin-text)', fontSize: '1.125rem', fontWeight: 600 }}>{isNew ? 'Thêm dịch vụ' : 'Chỉnh sửa dịch vụ'}</h2>
