  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#f1f5f9' }}>Quản lý hình ảnh</h1>
          <p style={{ color: '#64748b', fontSize: '0.8125rem' }}>{files.length} file trong thư mục</p>
        </div>
        <label style={{
        ...
        {folders.map(f => (
          <button key={f.value} onClick={() => setCurrentFolder(f.value)}
            style={{
              ...
              background: currentFolder === f.value ? '#22c55e15' : '#1e293b',
              color: currentFolder === f.value ? '#22c55e' : '#94a3b8',
      ...
      {loading ? (
        <div style={{ color: '#94a3b8', padding: '2rem', textAlign: 'center' }}>Đang tải...</div>
      ) : files.length === 0 ? (
        <div style={{
          ...
          <p style={{ color: '#64748b' }}>Thư mục trống</p>
        </div>
      ) : (
          ...
          {files.map(f => (
            <div key={f} style={{
              ...
              <div style={{ padding: '0.5rem 0.625rem' }}>
                <p style={{
                  color: '#94a3b8', fontSize: '0.6875rem', overflow: 'hidden',
                ...
                <div style={{ display: 'flex', gap: '0.375rem' }}>
                  <button onClick={() => copyUrl(f)} style={{
                    flex: 1, background: '#334155', border: 'none', borderRadius: '5px',
                    padding: '4px', cursor: 'pointer', color: copied === f ? '#22c55e' : '#94a3b8',
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '1.5rem', maxWidth: '400px', width: '90%' }}>
            <h3 style={{ color: '#f1f5f9', marginBottom: '0.5rem' }}>Xóa file?</h3>
            <p style={{ color: '#94a3b8', fontSize: '0.875rem', marginBottom: '1.5rem' }}>File sẽ bị xóa vĩnh viễn.</p>
